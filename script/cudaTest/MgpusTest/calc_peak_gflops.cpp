...
int main(int argc, char **argv)
{
    ...
    for (int i = 0; i < dc; i++)
    {
        ...
        printf("FP32 Peak Performance = %.3f TFLOPS\n", cc2cores(prop.major, prop.minor) * prop.multiProcessorCount * (prop.clockRate / 1e6) * 2 / 1e3);
        if (has_fp16(prop.major, prop.minor))
        {
            printf("FP16 Peak Performance = %.3f TFLOPS\n", cc2cores(prop.major, prop.minor) * prop.multiProcessorCount * (prop.clockRate / 1e6) * 2 * 2 / 1e3);
        }
        if (has_fp16_hfma2(prop.major, prop.minor))
        {
            printf("FP16 Peak Performance = %.3f TFLOPS\n", cc2cores(prop.major, prop.minor) * prop.multiProcessorCount * (prop.clockRate / 1e6) * 2 * 4 / 1e3);
        }
        if (has_bf16(prop.major, prop.minor))
        {
            printf("BF16 Peak Performance = %.3f TFLOPS\n", cc2cores(prop.major, prop.minor) * prop.multiProcessorCount * (prop.clockRate / 1e6) * 2 * 2 / 1e3);
        }
        if (has_int8(prop.major, prop.minor))
        {
            printf("INT8 Peak Performance = %.3f TFLOPS\n", cc2cores(prop.major, prop.minor) * prop.multiProcessorCount * (prop.clockRate / 1e6) * 2 * 4 / 1e3);
        }
        if (has_tensor_core_v1(prop.major, prop.minor))
        {
            printf("Tensor Core FP16 Peak Performance = %.3f TFLOPS\n", cc2cores(prop.major, prop.minor) * prop.multiProcessorCount * (prop.clockRate / 1e6) * 2 * 8 / 1e3);
        }
        if (has_tensor_core_v2(prop.major, prop.minor))
        {
            printf("Tensor Core FP16 Peak Performance = %.3f TFLOPS\n", cc2cores(prop.major, prop.minor) * prop.multiProcessorCount * (prop.clockRate / 1e6) * 2 * 8 / 1e3);
            printf("Tensor Core INT8 Peak Performance = %.3f TFLOPS\n", cc2cores(prop.major, prop.minor) * prop.multiProcessorCount * (prop.clockRate / 1e6) * 2 * 16 / 1e3);
        }
        if (has_tensor_core_v3(prop.major, prop.minor))
        {
            printf("Tensor Core TF32 Peak Performance = %.3f TFLOPS\n", cc2cores(prop.major, prop.minor) * prop.multiProcessorCount * (prop.clockRate / 1e6) * 2 * 8 / 1e3);
            printf("Tensor Core BF16 Peak Performance = %.3f TFLOPS\n", cc2cores(prop.major, prop.minor) * prop.multiProcessorCount * (prop.clockRate / 1e6) * 2 * 16 / 1e3);
            printf("Tensor Core FP16 Peak Performance = %.3f TFLOPS\n", cc2cores(prop.major, prop.minor) * prop.multiProcessorCount * (prop.clockRate / 1e6) * 2 * 16 / 1e3);
            printf("Tensor Core INT8 Peak Performance = %.3f TFLOPS\n", cc2cores(prop.major, prop.minor) * prop.multiProcessorCount * (prop.clockRate / 1e6) * 2 * 32 / 1e3);
        }
    }
    return 0;
}
